...
function ChampionCard({ userId, league, teams, existingTeamId }: {
  userId: string
  ...
  async function handleSave() {
    ...
    await supabase.from('champion_predictions').insert({
      user_id: userId, league_id: league.id, team_id: selected,
    })
    setSaving(false)
    setSaved(true)
  }
  ...
  return (
    <div className="rounded-2xl overflow-hidden"
      ...
      <div className="flex items-center justify-between px-4 py-2.5"
        ...
        <span className="text-xs font-black uppercase tracking-wide">{league.name}</span>
        {locked
      ...
      <div className="px-4 py-4">
        {locked && selectedTeam ? (
          /* Campeón elegido — solo visualización */
...
export default function ChampionSection({ userId, leagues, teams, champMap }: Props) {
      ...
      <div className="space-y-3">
        {leagues.map(league => (
          <ChampionCard
            ...
            league={league}
            teams={teams}
            existingTeamId={champMap[league.id] ?? null}
          />
        ))}
      </div>
    </div>
